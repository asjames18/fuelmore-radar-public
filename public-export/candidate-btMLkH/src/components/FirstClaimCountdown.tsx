 ...
 *
 * Counts down to 00:00 UTC on the first date with scheduled maturities —
 * the earliest moment the first FUEL rewards can unlock. When matured
 * positions are already waiting (`due > 0`), it flips to a "claims are
 * live" state instead of a timer. Quiet (renders nothing) when the
 * maturity schedule is unavailable, so a feed hiccup never blanks Overview.
 */
...
export function FirstClaimCountdown({ maturity }: { maturity: MaturityReport | undefined }) {
  ...
  const target = Date.parse(`${next.date}T00:00:00Z`)
  ...
  if (diff <= 0) {
    return <div className="claim-countdown live" role="status" aria-label="First FUEL rewards are unlocking now">
      ...
      <div className="claim-countdown-sub">First rewards maturing today — {next.scheduled.toLocaleString('en-US')} {next.scheduled === 1 ? 'position' : 'positions'} scheduled.</div>
    </div>
  }
  ...
  return <div className="claim-countdown" role="timer" aria-label={`First FUEL rewards unlock in ${digits}`}>
    ...
    <div className="claim-countdown-sub">Earliest unlock · {formatDate(next.date)} · {next.scheduled.toLocaleString('en-US')} {next.scheduled === 1 ? 'position' : 'positions'} maturing that day</div>
  </div>
}
