...
const data=():RadarData=>({updatedAt:new Date().toISOString(),partial:false,pairs:[],contracts:[],activity:[],holders:{FUEL:unavailableHolders(),MORE:unavailableHolders()},sources:SOURCE_NAMES.map(name=>({name,status:'available',checkedAt:new Date().toISOString(),detail:''})),protocol:Object.fromEntries(['totalSupply','globalRank','activeMinters','totalStaked','activeStakes','amp','eaar','maxTermSeconds','fuelBurnt','moreBurnt','ethUsedFuelBurns','ethUsedMoreBurns','totalDistributed','vaultBalance','vaultSwept','vaultCycle','vaultCycleEnd'].map(k=>[k,999n])) as ProtocolSnapshot})
