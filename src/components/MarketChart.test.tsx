...
it('renders our own candle panes for both tokens with Dexscreener fallback links',()=>{
 setup()
...
it('offers an actual-dollar overlay and optional percentage comparison, then dollar liquidity',()=>{
 ...
 expect(screen.queryByText('FUEL / USD')).toBeNull()
...
it('narrows the candle view to a single token',()=>{
 setup();fireEvent.click(screen.getByRole('button',{name:'FUEL'}))
 expect(screen.getByText('FUEL / USD')).toBeTruthy()
 expect(screen.queryByText('MORE / USD')).toBeNull()
